...
describe('serialize', () => {
    ...
    it('should serialize a single metric with labels', () => {
        expect(serialize([{ name: 'foo', value: 123, labels: { bar: 'baz' } }])).toEqual('foo{bar="baz"} 123');
    });

    it('should serialize a single metric with timestamp', () => {
        expect(serialize([{ name: 'foo', value: 123, timestamp: 12345 }])).toEqual('foo 123 12345');
    });

    it('should serialize a single metric with type', () => {
        expect(serialize([{ name: 'foo', value: 123, type: 'gauge' }])).toEqual(`# TYPE foo gauge
foo 123`);
    });

    it('should serialize a single metric with help', () => {
        expect(serialize([{ name: 'foo', value: 123, help: 'This is a foo metric' }])).toEqual(`# HELP foo This is a foo metric
foo 123`);
    });

    it('should serialize a single metric with single-line comment', () => {
        expect(serialize([{ name: 'foo', value: 123, comment: 'hello there' }])).toEqual(`# hello there
foo 123`);
    });
...
foo 123`);
    ...
    it('should serialize multiple metrics without details', () => {
        expect(serialize([{ name: 'foo', value: 123 }, { name: 'bar', value: 456 }])).toEqual(`foo 123
bar 456`);
    });
