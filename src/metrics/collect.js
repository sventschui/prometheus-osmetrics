    ...
    await Promise.all(
      pod.spec.containers.map(async container => {
        ...
        if (response.status === 204) {
          console.log({ pod })
          console.error(
            `Could not fetch CPU usage_rate for container ${container.name} of pod ${pod.metadata.uid} (${pod.metadata.name})`
          )
          return null
        }
  ...
  return (
    await Promise.all(
      pod.spec.containers.map(async container => {
        ...
        if (body.length <= 0) {
          console.error(
            `Could not fetch Memory usage for container ${container.name} of pod ${pod.metadata.uid} (${pod.metadata.name})`
          )
          return null
        }
